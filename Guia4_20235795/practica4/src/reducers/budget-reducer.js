...
export const budgetReducer = (state, action) => {
    switch (action.type) {
        ...
        case "update-expense":
            return {
                ...state, expenses: state.expenses.map(expense => expense.id === action.payload.expense.id? action.payload.expense: expense), modal:false, editingId:""
            }
        case "add-filter-category":
            return{...state, currentCategory:action.payload.categoryId}
        case "reset-app":
            return {...state, expenses: [], editingId: '', budget: 0}
        default:
            return state;
    }
}
